...
internal class Popup: PopupWrapper {
    ...
    private var downloadColorState: SColor = .indigo
    ...
    private var uploadColorState: SColor = .amber
    private var uploadColor: NSColor {
        var value = NSColor.seriesAmber
    ...
    private func initDetails() -> NSView {
        ...
        totalUpload.0.set(direction: "arrow.up")
        totalDownload.0.set(direction: "arrow.down")
    ...
    private func topValueView(_ view: NSView, title: String, color: NSColor) -> (NSView, NSTextField, NSTextField, ColorView) {
        ...
        // Down for what arrives, up for what leaves -- the marker now states
        // the direction instead of leaving it to the colour and the caption.
        let isDownload = title == localizedString("Downloading")
        let colorBlock: ColorView = ColorView(
            frame: NSRect(x: 0, y: 2, width: iconSize, height: iconSize),
            color: color, radius: 4,
            direction: isDownload ? "arrow.down" : "arrow.up")
    ...
    private func setUploadDownloadFields() {
        ...
        // Not just on/off: the marker tracks the rate, so a glance at the
        // popup says how busy the link is before you read the figure.
        self.uploadStateView?.setIntensity(seriesIntensity(forRate: Double(self.uploadValue)))
        self.downloadStateView?.setIntensity(seriesIntensity(forRate: Double(self.downloadValue)))
    }
