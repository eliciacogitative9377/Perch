...
public class Portal: PortalWrapper {
    ...
    private var downloadColor: NSColor {
        let v = SColor.fromString(Store.shared.string(key: "\(self.name)_downloadColor", defaultValue: SColor.indigo.key))
        var value = NSColor.systemIndigo
    ...
    private var uploadColor: NSColor {
        let v = SColor.fromString(Store.shared.string(key: "\(self.name)_uploadColor", defaultValue: SColor.amber.key))
    ...
    private func ioView() -> NSView {
        ...
        let upload = portalWithColorRow(view, color: self.uploadColor, title: "\(localizedString("Uploading")):")
        let download = portalWithColorRow(view, color: self.downloadColor, title: "\(localizedString("Downloading")):")
        (upload.0 as? ColorBlock)?.set(direction: "arrow.up")
        (download.0 as? ColorBlock)?.set(direction: "arrow.down")
        self.uploadField = upload.1
        self.downloadField = download.1
