...
internal class Popup: PopupWrapper {
    ...
    private var mainColor: NSColor { self.mainColorState.additional as? NSColor ?? NSColor.perchAccent }
    private var readColorState: SColor = .indigo
    private var readColor: NSColor { self.readColorState.additional as? NSColor ?? NSColor.systemIndigo }
    private var writeColorState: SColor = .amber
    private var writeColor: NSColor { self.writeColorState.additional as? NSColor ?? NSColor.perchAccent }
...
internal class ChartView: NSStackView {
    ...
    /// Reads arrive, writes leave -- the same arrow language the Network
    /// module uses, so one glance means the same thing in both popups.
    private var markersSet = false
    ...
    private var readColor: NSColor {
        SColor.fromString(Store.shared.string(key: "\(ModuleType.disk.stringValue)_readColor", defaultValue: SColor.indigo.key)).additional as! NSColor
    }
    private var writeColor: NSColor {
        SColor.fromString(Store.shared.string(key: "\(ModuleType.disk.stringValue)_writeColor", defaultValue: SColor.amber.key)).additional as! NSColor
    }
    private var base: DataSizeBase {
    ...
    public init(width: CGFloat) {
        ...
        if !self.markersSet {
            self.readColorView.set(direction: "arrow.down")
            self.writeColorView.set(direction: "arrow.up")
