...
internal class Preview: PreviewWrapper {
    ...
    private var swapTile: StatTileView? = nil
    ...
    private var chartColor: NSColor { self.chartColorState.additional as? NSColor ?? NSColor.perchAccent }
    ...
    private func pressureView() -> NSView {
        ...
        // The same rainbow needle the popup had, in a fourth file. A hue ramp
        // is not a magnitude scale, and a speedometer is a lot of pixels for
        // one of three words.
        let pill = StatPillView(caption: localizedString("Memory pressure"))
        pill.widthAnchor.constraint(equalToConstant: 110).isActive = true
        pill.toolTip = localizedString("Memory pressure")
        self.pressurePill = pill
    ...
    private func swapView() -> NSView {
        ...
        // Swap has a total, so a meter is honest here -- an arc still is not.
        let tile = StatTileView(caption: localizedString("Swap"))
        tile.widthAnchor.constraint(equalToConstant: 110).isActive = true
        tile.toolTip = localizedString("Swap")
        self.swapTile = tile
    ...
    public func loadCallback(_ value: RAM_Usage) {
        DispatchQueue.main.async(execute: {
            if (self.window?.isVisible ?? false) || !self.initialized {
                ...
                let swapFraction = value.swap.total > 0 ? value.swap.used/value.swap.total : 0
                self.swapTile?.set(value: Units(bytes: Int64(value.swap.used)).getReadableMemory(style: .memory),
                                   fraction: value.swap.total > 0 ? swapFraction : nil,
                                   color: swapFraction.usageColor())
