...
internal class Popup: PopupWrapper {
    ...
    private var pressurePill: StatPillView? = nil
    ...
    private var appColorState: SColor = .chartBlue
    private var appColor: NSColor { self.appColorState.additional as? NSColor ?? NSColor.systemRed }
    private var wiredColorState: SColor = .chartMagenta
    private var wiredColor: NSColor { self.wiredColorState.additional as? NSColor ?? NSColor.perchAccent }
    private var compressedColorState: SColor = .chartOlive
    ...
    private var freeColor: NSColor { self.freeColorState.additional as? NSColor ?? NSColor.perchAccent }
    ...
    private func initDashboard() -> NSView {
        ...
        // The figure and where it sits in its range, instead of an arc the
        // reader has to estimate by angle.
        let usage = StatTileView(caption: localizedString("Memory usage"))
        usage.toolTip = localizedString("Memory usage")
        self.usageTile = usage

        // One of three words, in a coloured capsule. The needle it replaces
        // spent 70x60 points saying "Normal".
        let pressure = StatPillView(caption: localizedString("Memory pressure"))
        pressure.toolTip = localizedString("Memory pressure")
        self.pressurePill = pressure

        view.addArrangedSubview(usage)
        view.addArrangedSubview(pressure)
    ...
    private func renderLoad(_ value: RAM_Usage) {
        ...
        self.usageTile?.set(value: "\(Int(value.usage*100))%",
                            fraction: value.usage,
                            color: value.usage.usageColor())
        self.usageTile?.toolTip = "\(localizedString("Memory usage")): \(Int(value.usage*100))% · \(Units(bytes: Int64(value.used)).getReadableMemory()) \(localizedString("used"))"
        
        // Pressure has its own three levels, so it wears the load ramp's
        // steps rather than a second set of colours nobody calibrated.
        let pressureColor: NSColor
        switch value.pressure.value {
        case .normal:   pressureColor = .loadCalm
        case .warning:  pressureColor = .loadWarning
        case .critical: pressureColor = .loadCritical
        }
        self.pressurePill?.set(text: localizedString(value.pressure.value.rawValue.capitalized),
                               color: pressureColor)
        self.pressurePill?.toolTip = "\(localizedString("Memory pressure")): \(value.pressure.value.rawValue)"
