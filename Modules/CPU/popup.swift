...
internal class Popup: PopupWrapper {
    ...
    private var temperatureTile: StatTileView? = nil
    private var frequencyTile: StatTileView? = nil
    ...
    private var userColor: NSColor { self.userColorState.additional as? NSColor ?? NSColor.perchAccent }
    ...
    /// Utilization by default, so the chart states the load in its colour as
    /// well as its height -- the accent colour said the same thing at 3% and
    /// at 97%.
    private var chartColorState: SColor = .utilization
    /// The last value seen, for resolving the utilization colour.
    private var chartValue: Double = 0
    private var chartColor: NSColor {
        if self.chartColorState.key == SColor.utilization.key {
            return self.chartValue.usageColor()
        }
        return self.chartColorState.additional as? NSColor ?? NSColor.perchAccent
    }
    ...
    private var pCoresColor: NSColor { self.pCoresColorState.additional as? NSColor ?? NSColor.perchAccent }
    ...
    private func initDashboard() -> NSView {
        ...
        view.edgeInsets = NSEdgeInsets(top: 8, left: 0, bottom: 4, right: 0)

        // Temperature and frequency start hidden, as they did before: not
        // every Mac reports them, and an empty tile is worse than no tile.
        let temperature = StatTileView(caption: localizedString("CPU temperature"))
        temperature.toolTip = localizedString("CPU temperature")
        temperature.isHidden = true
        self.temperatureTile = temperature

        let usage = StatTileView(caption: localizedString("CPU usage"))
        usage.toolTip = localizedString("CPU usage")
        self.usageTile = usage

        let frequency = StatTileView(caption: localizedString("CPU frequency"))
        frequency.toolTip = localizedString("CPU frequency")
        frequency.isHidden = true
        self.frequencyTile = frequency

        view.addArrangedSubview(temperature)
    ...
    public func loadCallback(_ value: CPU_Load) {
        ...
        self.chartValue = value.totalUsage
        self.lineChart?.addValue(value.totalUsage)
        // Recolour as it goes: without this the chart keeps whatever colour
        // the load happened to be when the popup opened.
        self.lineChart?.setColor(self.chartColor)
    }
    ...
    private func renderLoad(_ value: CPU_Load) {
        ...
        let percent = Int(value.totalUsage.rounded(toPlaces: 2) * 100)
        self.usageTile?.set(value: "\(percent)%",
                            fraction: value.totalUsage,
                            color: value.totalUsage.usageColor())
        self.usageTile?.toolTip = "\(localizedString("CPU usage")): \(percent)% · \(localizedString("System")) \(Int(value.systemLoad*100))% · \(localizedString("User")) \(Int(value.userLoad*100))%"
    ...
    private func renderTemperature(_ value: Double) {
        if let view = self.temperatureTile, view.isHidden {
            view.isHidden = false
        }
        
        // No meter: a CPU has no published maximum to measure a reading
        // against, and the old ring filled an arbitrary fraction of a circle
        // as though 100C were the top of a scale. The figure stands alone, and
        // the thermal zones colour it.
        self.temperatureTile?.set(value: temperature(value),
                                  fraction: nil,
                                  color: (value/100).usageColor(zones: (0.65, 0.85)))
        self.temperatureTile?.toolTip = "\(localizedString("CPU temperature")): \(temperature(value))"
    }
    ...
    private func renderFrequency(_ value: CPU_Frequency) {
        ...
        if let view = self.frequencyTile, view.isHidden {
            view.isHidden = false
        }
        ...
        if let v = value.value {
            ...
            if let tile = self.frequencyTile {
                // Against the highest frequency seen so far, which is the only
                // ceiling available -- so the meter is honest about being
                // relative, and the figure carries the absolute value.
                let fraction = self.maxFreq > 0 ? v/self.maxFreq : 0
                tile.set(value: "\((v/1000).rounded(toPlaces: 2)) GHz",
                         fraction: fraction,
                         color: .loadCalm)
                tile.toolTip = "\(localizedString("CPU frequency")): \(Int(v)) MHz - \((fraction*100).rounded(toPlaces: 2))%"
            }
        }
