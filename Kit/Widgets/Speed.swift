...
public class SpeedWidget: WidgetWrapper {
    /// Arrows, not dots. The arrow style was already built and unused, while
    /// two coloured circles make the reader remember which colour was which
    /// every time they glance up. Direction is the one thing the marker can
    /// say for itself. Modules that want letters set Icon in their config.
    private var icon: String = "arrows"
    ...
    private var inputColorState: SColor = .indigo
    private var outputColorState: SColor = .amber
    ...
    /// Below one threshold the marker used to go grey and above it fully
    /// coloured, so 2 KB/s and 20 MB/s looked identical. Colour is the only
    /// thing this marker can carry beyond direction, so it carries magnitude.
    private func ramped(_ color: NSColor, _ bytes: Int64) -> NSColor {
        seriesColor(color, forRate: Double(bytes))
    }
    
    private var inputColor: (String) -> NSColor {{ state in
        if state == "none" { return .textColor }
        let color = self.monochromeState ? MonochromeColor.blue : (self.inputColorState.additional as? NSColor ?? NSColor.perchAccent)
        ...
        return self.ramped(color, self.inputValue)
    }}
    private var outputColor: (String) -> NSColor {{ state in
        if state == "none" { return .textColor }
        let color = self.monochromeState ? MonochromeColor.red : (self.outputColorState.additional as? NSColor ?? NSColor.seriesAmber)
        ...
        return self.ramped(color, self.outputValue)
    }}
