...
/// Perch's own accent.
///
/// The fork's charts and widgets default to this instead of systemBlue, so
/// the app reads as itself rather than as stock macOS blue. The named
/// palette below is left alone on purpose: when a user picks "Blue" they
/// should get blue, not a rebranded approximation.
public extension NSColor {
    static let perchAccent = NSColor(srgbRed: 0.31, green: 0.78, blue: 0.94, alpha: 1)
    static let perchAccentDeep = NSColor(srgbRed: 0.09, green: 0.16, blue: 0.38, alpha: 1)
}
...
extension SColor: CaseIterable {
    ...
    /// The outbound series colour: upload, and disk writes.
    ///
    /// Not systemOrange, which is too light to sit in the same lightness band
    /// as the inbound blue -- one series would shout over the other. This step
    /// was measured against both chart surfaces rather than picked by eye:
    /// OKLab lightness inside the band in light and dark, chroma above the
    /// floor, and separation from the inbound blue of dE 30.8 under
    /// protanopia and 26.5 under tritanopia, well past the 8 required.
    public static var amber: SColor { SColor(key: "amber", value: "Amber", additional: NSColor.seriesAmber) }
    /// The validated categorical trio, for breakdowns of a total.
    public static var chartBlue: SColor { SColor(key: "chartBlue", value: "Chart blue", additional: NSColor.chartBlue) }
    public static var chartMagenta: SColor { SColor(key: "chartMagenta", value: "Chart magenta", additional: NSColor.chartMagenta) }
    public static var chartOlive: SColor { SColor(key: "chartOlive", value: "Chart olive", additional: NSColor.chartOlive) }
    ...
    public static var allCases: [SColor] {
        return [.utilization, .pressure, .cluster, separator1,
                ...
                .orange, .secondOrange, .amber, .chartBlue, .chartMagenta, .chartOlive,
                .purple, .secondPurple, .brown, .secondBrown,
