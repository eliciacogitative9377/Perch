...
public class LabelField: NSTextField {
    public init(frame: NSRect = NSRect.zero, _ label: String = "", size: CGFloat = 12) {
        ...
        // The label names the row; the value is what you came to read. Letting
        // the label recede is what makes a column of twenty rows scannable.
        self.alphaValue = 0.9
...
public class ValueField: NSTextField {
    public init(frame: NSRect = NSRect.zero, _ value: String = "") {
        ...
        // Monospaced digits, because every one of these is a number that
        // changes once a second: in a proportional font "711.4 MB" and
        // "3.48 GB" are different widths, so the whole column twitches.
        self.font = NSFont.monospacedDigitSystemFont(ofSize: 12, weight: .medium)
...
public class StatusBadgeView: NSStackView {
    ...
    /// The tint the badge is built from.
    private var tint: NSColor {
        if let status {
            return status ? .systemGreen : .systemRed
        }
        return .secondaryLabelColor
    }
    /// A solid block of saturated green shouts louder than the reading beside
    /// it. A tinted capsule with coloured text carries the same information at
    /// the weight it deserves.
    private var color: CGColor {
        self.tint.withAlphaComponent(0.16).cgColor
    }
    ...
    public init(frame: NSRect = NSRect(origin: .zero, size: NSSize(width: 50, height: 14)), _ status: Bool? = nil, ok: String = "UP", notOk: String = "DOWN") {
        ...
        self.spacing = 0
        self.wantsLayer = true
        self.layer?.cornerRadius = frame.height/2      // capsule
        self.layer?.backgroundColor = self.color
        
        self.labelField.autoresizingMask = [.width, .height]
        self.labelField.alignment = .center
        self.labelField.textColor = self.tint
        self.labelField.stringValue = self.label
        self.labelField.alphaValue = 1
        self.labelField.font = NSFont.systemFont(ofSize: 10, weight: .semibold)
...
/// How strongly to paint a direction marker for a byte rate, 0...1.
///
/// One place, because the menu bar widget and the popups have to agree: a
/// marker that means "busy" in the menu bar and "any traffic at all" in the
/// popup teaches the reader nothing.
///
/// Log-scaled. Network and disk rates span four orders of magnitude, so on a
/// linear ramp everything under a megabyte would sit at the bottom and the
/// whole scale would be spent on speeds you rarely reach. Here a decade of
/// rate is about a fifth of the ramp.
public func seriesIntensity(forRate bytesPerSecond: Double) -> CGFloat {
    let floor: Double = 1024                // 1 KB/s and below reads as idle
    let ceiling: Double = 10*1024*1024      // 10 MB/s is full strength
    guard bytesPerSecond > floor else { return 0 }
    let t = log(bytesPerSecond / floor) / log(ceiling / floor)
    return CGFloat(min(max(t, 0), 1))
}

public extension NSColor {
    /// This series colour at a given intensity, 0...1.
    ///
    /// A sequential ramp in one hue: quiet and dark when idle, the base colour
    /// mid-range, brighter and more vivid at the top. The first attempt only
    /// blended 40% toward mid-grey, which moves indigo from #6C6ABD to #5E5CE6
    /// -- a span you cannot see. This one travels far enough to read at a
    /// glance while the hue never moves, which is what keeps it a magnitude
    /// encoding rather than a rainbow.
    ///
    /// Two stops, not one: idle -> base over the first half, base -> peak over
    /// the second. A single lerp from idle to base wastes half the range on
    /// colours dimmer than the one the user actually chose.
    ///
    /// Direction follows the appearance, because "busy" means high contrast
    /// against the surface: on a dark menu bar that is brighter, on a light one
    /// darker. A ramp flipped automatically would put idle at maximum contrast
    /// in one of the two modes.
    ///
    /// Measured at the ends (dark): idle #34337F / #714400 stay 20.5 apart
    /// under protanopia and sit at 1.6-2.1 contrast -- present but quiet;
    /// peak #8E8DEE / #DDA34C are 25.8 apart and clear 3:1. The categorical
    /// lightness band does not apply here: traversing lightness is the whole
    /// point of a sequential scale.
    func atIntensity(_ fraction: CGFloat) -> NSColor {
        guard let full = self.usingColorSpace(.deviceRGB) else { return self }
        ...
        return full.blended(withFraction: (t - 0.5) * 2, of: peak) ?? full
    }
}

/// A series colour that follows the rate behind it.
public func seriesColor(_ base: NSColor, forRate bytesPerSecond: Double) -> NSColor {
    base.atIntensity(seriesIntensity(forRate: bytesPerSecond))
}
...
public class ColorView: NSView {
    ...
    /// An arrow instead of a filled shape, for series that have a direction.
    /// Shape carries the identity alongside the colour, so download and upload
    /// stay apart for a reader who cannot separate the two hues -- and an arrow
    /// says which way the bytes are going, which a dot never did.
    private var arrow: NSImageView?

    public init(frame: NSRect = NSRect.zero, color: NSColor, state: Bool = false, radius: CGFloat = 2, direction: String? = nil) {
        self.color = color
        self.state = state
        ...
        if let direction {
            ...
            let image = NSImageView(frame: self.bounds)
            image.imageScaling = .scaleProportionallyUpOrDown
            image.image = NSImage(systemSymbolName: direction, accessibilityDescription: nil)?
                .withSymbolConfiguration(.init(pointSize: frame.height, weight: .bold))
            image.contentTintColor = state ? self.color : self.inactiveColor
    ...
    public func setState(_ newState: Bool) {
        if newState != state {
            if let arrow {
                arrow.contentTintColor = newState ? self.color : self.inactiveColor
            } else {
    ...
    /// Paint the marker in proportion to the rate behind it, instead of
    /// flipping between two states. Idle stays visible but quiet, so a row
    /// does not blink out of existence between packets.
    public func setIntensity(_ fraction: CGFloat) {
        let clamped = min(max(fraction, 0), 1)
        let tint = self.color.atIntensity(clamped)
        if let arrow {
            arrow.contentTintColor = tint
        } else {
            self.layer?.backgroundColor = tint.cgColor
        }
        self.state = clamped > 0
    }
    ...
    public func setColor(_ newColor: NSColor) {
        ...
        self.color = newColor
        if let arrow {
            if self.state { arrow.contentTintColor = newColor }
        } else {
...
public class SMCHelper {
    ...
    private let id: String = "com.sagar.perch.SMC.Helper"
    private let plistName: String = "com.sagar.perch.SMC.Helper.plist"
    ...
    public func checkForUpdate() {
        ...
        let helperURL = Bundle.main.bundleURL.appendingPathComponent("Contents/Library/LaunchServices/com.sagar.perch.SMC.Helper")
    ...
    private func installLegacy(completion: @escaping (_ state: SMCHelperInstallState) -> Void) {
        ...
        if SMJobBless(kSMDomainSystemLaunchd, "com.sagar.perch.SMC.Helper" as CFString, authRef, &error) == false {
    ...
    private func helperConnection() -> NSXPCConnection? {
        ...
        let connection = NSXPCConnection(machServiceName: "com.sagar.perch.SMC.Helper", options: .privileged)
...
public class ColorBlock: NSView {
    private var arrow: NSImageView?
    private var tint: NSColor = .lightGray.withAlphaComponent(0.75)
    ...
    public init(frame: CGRect? = nil, toolTip: String? = nil, color: NSColor? = nil) {
        ...
        self.wantsLayer = true
        self.tint = color ?? .lightGray.withAlphaComponent(0.75)
        self.layer?.backgroundColor = self.tint.cgColor
        // Round, not a rounded square: these sit beside text as legend dots,
        // and a circle reads as a marker rather than a truncated swatch.
        self.layer?.cornerRadius = (frame?.height ?? 10)/2
    ...
    public func set(color: NSColor? = nil) {
        ...
        self.effectiveAppearance.performAsCurrentDrawingAppearance {
            if let arrow = self.arrow {
                arrow.contentTintColor = resolved
            } else {
    ...
    /// Swap the dot for a direction arrow. Shape then carries the identity
    /// alongside colour, and the marker says which way the bytes move.
    public func set(direction: String?) {
        ...
        image.contentTintColor = self.tint
        image.autoresizingMask = [.width, .height]
        self.addSubview(image)
        self.arrow = image
    }
