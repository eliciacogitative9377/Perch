...
public class DB {
    ...
    private let queue = DispatchQueue(label: "com.sagar.perch.db")
    ...
    init() {
        ...
        let supportPath = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!.appendingPathComponent("Perch")
        let tmpPath = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("Perch")
