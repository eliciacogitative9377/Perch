...
public class SystemStats {
    ...
    // Upstream's endpoints are gone. The accounts, the MQTT broker and the
    // protocol behind them are the original author's infrastructure, and a fork
    // pointing at them would sign its users into someone else's service. These
    // now read from Branding, which ships with no host -- so start() is a
    // no-op and nothing here ever dials out. Fill in Branding.remoteServiceHost
    // to run this against infrastructure of your own.
    static public var host = Branding.remoteServiceHost ?? URL(string: "https://localhost.invalid")!
    static public var authHost = Branding.remoteServiceHost ?? URL(string: "https://localhost.invalid")!
    static public var brokerHost = URL(string: "wss://localhost.invalid/mqtt")!
    static public var appHost = Branding.remoteServiceHost ?? URL(string: "https://localhost.invalid")!
    ...
    public func login() {
        self.auth.login { url in
            ...
            debug("Open \(url) to login to Perch Remote", log: self.log)
    ...
    public func logout() {
        ...
        debug("Logout successfully from Perch Remote", log: self.log)
    ...
    @objc private func successLogin() {
        ...
        NotificationCenter.default.post(name: .remoteState, object: nil, userInfo: ["auth": self.isAuthorized])
        self.mqtt.connect()
        debug("Login successfully on Perch Remote", log: self.log)
    }
    
    public func start() {
        // No service configured: never reach for the network.
        guard Branding.remoteServiceHost != nil else { return }
...
class MQTTManager: NSObject {
    ...
    private let stateQueue = DispatchQueue(label: "com.sagar.perch.Remote.MQTT")
...
enum RemoteKeychain {
    private static let service: String = (Bundle.main.bundleIdentifier ?? "com.sagar.perch") + ".remote"
