...
internal class SetupWindow: NSWindow, NSWindowDelegate {
    ...
    init() {
        ...
        self.delegate = self
        self.title = localizedString("Perch Setup")
...
private class SetupView_welcome: NSStackView {
    init() {
        ...
        title.stringValue = localizedString("Welcome to Perch")
        title.toolTip = localizedString("Welcome to Perch")
...
private class SetupView_update: NSStackView {
    private var value: AppUpdateInterval {
        get {
            let value = Store.shared.string(key: "update-interval", defaultValue: Branding.defaultUpdateInterval)
...
private class SetupView_end: NSStackView {
    init() {
        ...
        // Upstream's System Stats, Sponsors, PayPal, Ko-fi and Patreon buttons
        // are gone: every one of them paid the original author rather than this
        // fork, and the first advertised a service this fork cannot serve. Set
        // Branding.donationURL to put a button of your own back.
        if let repository = Branding.repositoryURL {
            support.addArrangedSubview(SupportButtonView(name: "GitHub", image: "github", action: {
                NSWorkspace.shared.open(repository)
            }))
        }
        if let donation = Branding.donationURL {
            support.addArrangedSubview(SupportButtonView(name: "Donate", image: "AppIcon", action: {
