...
class ApplicationSettings: NSStackView {
    private var updateIntervalValue: String {
        Store.shared.string(key: "update-interval", defaultValue: Branding.defaultUpdateInterval)
    }
    ...
    private var systemWidgetsUpdatesState: Bool {
        get {
            let userDefaults = UserDefaults(suiteName: "\(Bundle.main.object(forInfoDictionaryKey: "TeamId") as! String).com.sagar.perch.widgets")
    ...
    init() {
        ...
        self.remoteView = PreferencesSection(title: "System Perch", [
        ...
        // Only when there is a service to sign into. The Remote module is not
        // loaded and Branding.remoteServiceHost is nil, so "Sign in" would
        // open an OAuth flow against a host that does not answer -- a button
        // that can only fail is worse than no button.
        if Branding.remoteServiceHost != nil {
    ...
    @objc private func exportSettings() {
        ...
        panel.nameFieldStringValue = "Perch.plist"
