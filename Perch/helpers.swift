...
extension AppDelegate {
    ...
    internal func parseVersion() {
        ...
        guard let updateInterval = AppUpdateInterval(rawValue: Store.shared.string(key: "update-interval", defaultValue: Branding.defaultUpdateInterval)) else {
            return
        }
        ...
        if !Store.shared.exist(key: key) {
            ...
            if updateInterval != .silent && isNewestVersion(currentVersion: prevVersion, latestVersion: currentVersion) {
                ...
                let subtitle: String = localizedString("Perch was updated to v", currentVersion)
    ...
    internal func defaultValues() {
        ...
        if let updateInterval = AppUpdateInterval(rawValue: Store.shared.string(key: "update-interval", defaultValue: Branding.defaultUpdateInterval)) {
    ...
    private func showUpdateNotification(version: version_s) {
        ...
        _ = showNotification(
            ...
            subtitle: localizedString("Click to install the new version of Perch"),
    ...
    internal func icon() {
        if self.pauseState {
            ...
            DispatchQueue.main.async(execute: {
                self.menuBarItem?.autosaveName = "Perch"
            })
