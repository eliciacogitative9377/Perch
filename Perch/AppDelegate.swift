...
// Remote() is deliberately absent: it is a client for the original author's
// hosted service (accounts, MQTT broker, device registry), so a fork has
// nothing to connect it to. See Branding.remoteServiceHost.
...
class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    internal let updateActivity = NSBackgroundActivityScheduler(identifier: "com.sagar.perch.updateCheck")
    internal let supportActivity = NSBackgroundActivityScheduler(identifier: "com.sagar.perch.support")
    internal let supportRetryActivity = NSBackgroundActivityScheduler(identifier: "com.sagar.perch.supportRetry")
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        ...
        info("Perch started in \((startingPoint.timeIntervalSinceNow * -1).rounded(toPlaces: 4)) seconds")
        self.startTS = Date()
    }
            ...
            if let url = URL(string: uri) {
                updater.download(url, completion: { path in
                    updater.install(path: path) { error in
                        if let error {
                            showAlert("Error update Perch", error, .critical)
                        }
                    }
                })
            }
