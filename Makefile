...
archive: clean
	osascript -e 'display notification "Exporting application archive..." with title "Build the Perch"'
...
notarize:
	osascript -e 'display notification "Submitting app for notarization..." with title "Build the Perch"'
	echo "Submitting app for notarization..."

	xcrun notarytool submit --keychain-profile "AC_PASSWORD" --wait $(ZIP_PATH)

	echo "Perch successfully notarized"

sign:
	osascript -e 'display notification "Stampling the Perch..." with title "Build the Perch"'
	echo "Going to staple an application..."

	xcrun stapler staple $(APP_PATH)
	spctl -a -t exec -vvv $(APP_PATH)

	osascript -e 'display notification "Perch successfully stapled" with title "Build the Perch"'
	echo "Perch successfully stapled"
...
verify:
	...
	marketingVersion=$$(/usr/libexec/PlistBuddy -c "print :objects:9A141107229E721200D29793:buildSettings:MARKETING_VERSION" "$(PWD)/Perch.xcodeproj/project.pbxproj") ;\
...
prepare-dmg:
	...
	./create-dmg/create-dmg \
	    --volname $(APP) \
	    --background "./Perch/Supporting Files/background.png" \
	    --window-pos 200 120 \
	    --window-size 500 320 \
	    --icon-size 80 \
	    --icon "Perch.app" 125 175 \
...
prepare-dSYM:
	echo "Zipping dSYMs..."
	cd $(BUILD_PATH)/Perch.xcarchive/dSYMs && zip -r $(PWD)/dSYMs.zip .
...
clean:
	...
	if [ -a $(PWD)/Perch.dmg ]; then rm $(PWD)/Perch.dmg; fi;

next-version:
	versionNumber=$$(/usr/libexec/PlistBuddy -c "Print CFBundleVersion" "$(PWD)/Perch/Supporting Files/Info.plist") ;\
	echo "Actual version is: $$versionNumber" ;\
	versionNumber=$$((versionNumber + 1)) ;\
	echo "Next version is: $$versionNumber" ;\
	/usr/libexec/PlistBuddy -c "Set :CFBundleVersion $$versionNumber" "$(PWD)/Perch/Supporting Files/Info.plist" ;\
...
open:
	osascript -e 'display notification "Perch signed and ready for distribution" with title "Build the Perch"'
